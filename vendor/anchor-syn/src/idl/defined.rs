    ...
    match ty {
        ...
        syn::Type::Path(path) => {
            ...
            #[cfg(procmacro2_semver_exempt)]
            {
                ...
                // `proc_macro2::Span::source_file()` is not available in some proc-macro2 versions.
                // For IDL builds we can locate the crate root reliably via `CARGO_MANIFEST_DIR`.
                ...
                let lib_path = source_path
                    ...
                    .join("lib.rs")
                    .exists()
                    .then(|| source_path.join("src").join("lib.rs"))
                    .or_else(|| find_path("lib.rs", &source_path).ok())
                    .expect("lib.rs should exist (CARGO_MANIFEST_DIR/src/lib.rs)");
